...
extend(AppView.prototype, {
    _init : function() {
        ...
        $(document)
            ...
            .on('click', '.segment .bus, .segment .trolley, .segment .tram, .segment .minibus', this._onSelectRoute.bind(this))
            .on('mouseover', '.segment .bus, .segment .trolley, .segment .tram, .segment .minibus', this._onRouteMouseOver.bind(this))
            .on('mouseout', '.segment .bus, .segment .trolley, .segment .tram, .segment .minibus', this._onRouteMouseOut.bind(this))
    ...
    _routeElemToRouteId : function(elem) {
        var elem = $(elem);
            ...
            routeType = elem.hasClass('trolley')? 'trolley': elem.hasClass('tram')? 'tram' : elem.hasClass('minibus')? 'minibus' : 'bus'

        return { trolley: 'Тб ', bus: '', tram : 'Тм ', minibus : 'Мт ' }[routeType] + routeNumber;
    },
    ...
    _onRoutesHighlightedOnMap : function(e, data) {
        ...
        var elementsToHighLight = $('.segment .bus, .segment .trolley, .segment .tram, .segment .minibus');

        data.routes.forEach(function(route) {
            var type = route.indexOf('Мт')? route.indexOf('Тб')? route.indexOf('Тм')? 'bus' : 'tram' : 'trolley' : 'minibus',
                routeCleared = route.replace(/^(Тб|Тм|Мт) /, '');
