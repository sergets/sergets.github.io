...
var SearchView = function(elem, dataManager) {
    ...
    this._suggest
        .on('click', '.bus, .trolley, .tram, .minibus', this._onRouteSelected.bind(this))
        .on('mouseover', '.bus, .trolley, .tram, .minibus', (function(e) { this._selectSuggestItem($(e.target)); }).bind(this));
};
...
extend(SearchView.prototype, {
    ...
    _mimicInput : function(elem) {
        if(elem && elem.length) {
            this._input
                ...
                .toggleClass('minibus', elem.hasClass('minibus'))
                .removeClass('search-route')
                .css('backgroundColor', elem.css('backgroundColor'));
        }
        else {
            this._input
                .removeClass('bus trolley tram minibus')
                .addClass('search-route')
                .css('backgroundColor', '');
        }
    },
    ...
    _routeElemToRouteId : function(elem) {
        var elem = $(elem);
            ...
            routeType = elem.hasClass('minibus')? 'minibus': elem.hasClass('trolley')? 'trolley': elem.hasClass('tram')? 'tram' : 'bus';
